...
const WORKFLOW_ID = '2086666604021493762';
...
function renderTaskQueue() {
  ...
  records.forEach((record) => {
    ...
    const summary = document.createElement('div'); const title = document.createElement('strong'); title.textContent = `一键消除 · ${record.status}`;
...
function updateOutputVisibility() {
  ...
  ui.button.querySelector('span').textContent = hasRunning ? `开始一键消除（${runningTaskKeys.size}/${MAX_CONCURRENT_TASKS}）` : hasImages ? '再次一键消除' : '开始一键消除';
  if (hasRunning) { setStatus('working', `${runningTaskKeys.size}/${MAX_CONCURRENT_TASKS} 个任务运行中`); ui.progressTitle.textContent = '正在清理画面…'; ui.progressDetail.textContent = '已完成的图片会持续显示在共享图片池中'; }
...
async function imageResultToCacheEntry(result) { const response = await fetch(result.url); if (!response.ok) throw new Error(`无法下载图片结果（HTTP ${response.status}）。`); return { name: `MiniMax-H3-Remove-${result.nodeId || 'result'}.${result.outputType || 'png'}`, blob: await response.blob() }; }
...
async function cacheImages(images) { for (const image of images) { if (!(image.blob instanceof Blob)) throw new Error('图片缓存数据无效，无法写入本地图片池。'); await poolRequest((store) => store.put({ id: imageId(), name: image.name, blob: image.blob, createdAt: Date.now(), source: 'MiniMax H3 一键消除' })); } }
...
function addImageCard(image) { const fragment = $('imageCardTemplate').content.cloneNode(true); const button = fragment.querySelector('.image-button'); const img = fragment.querySelector('img'); const download = fragment.querySelector('.download-image'); button.addEventListener('click', () => openLightbox(image)); img.src = image.url; img.alt = image.name; fragment.querySelector('.image-name').textContent = image.name; download.href = image.url; download.download = image.name; fragment.querySelector('.delete-image').addEventListener('click', async () => { await deleteCachedImage(image.id); URL.revokeObjectURL(image.url); await renderImagePool(); }); ui.poolGrid.append(fragment); }
async function renderImagePool() { const cached = await getCachedImages(); const invalid = cached.filter((image) => !(image.blob instanceof Blob)); await Promise.all(invalid.map((image) => deleteCachedImage(image.id))); const valid = cached.filter((image) => image.blob instanceof Blob); ui.poolGrid.querySelectorAll('img').forEach((image) => URL.revokeObjectURL(image.src)); ui.poolGrid.innerHTML = ''; ui.poolCount.textContent = `${valid.length} 张图片`; ui.poolEmpty.classList.toggle('hidden', valid.length > 0); valid.forEach((image) => addImageCard({ ...image, url: URL.createObjectURL(image.blob) })); updateOutputVisibility(); }
async function renderResults(task, record) { const outputs = task.results || []; if (!outputs.length) throw new Error('任务已完成，但未返回任何结果文件。'); const images = []; for (const result of outputs) { if (!result.url) continue; if (isZip(result)) { record.zipUrl = result.url; updateTaskRecord(record, '正在解压结果…', '浏览器正在读取 ZIP 中的图片'); images.push(...await unpackZip(result)); } else if (/\.(png|jpe?g|webp|gif|avif)(?:$|\?)/i.test(result.url)) images.push(await imageResultToCacheEntry(result)); } if (!images.length) throw new Error('任务已完成，但没有可预览的图片输出。'); await cacheImages(images); await renderImagePool(); updateTaskRecord(record, '消除完成', `${images.length} 张图片已加入共享图片池`, 'completed'); }

ui.file.addEventListener('change', () => {
  const file = ui.file.files[0]; if (!file) return;
  if (ui.preview.src) URL.revokeObjectURL(ui.preview.src);
  const objectUrl = URL.createObjectURL(file);
  ui.preview.src = objectUrl; ui.preview.classList.remove('hidden'); ui.uploadTitle.textContent = file.name;
  const dimensions = new Image();
  dimensions.onload = () => {
    ui.width.value = dimensions.naturalWidth;
    ui.height.value = dimensions.naturalHeight;
    ui.uploadDetail.textContent = `已选择 ${(file.size / 1024 / 1024).toFixed(1)} MB · ${dimensions.naturalWidth} × ${dimensions.naturalHeight}`;
    document.querySelectorAll('.size-presets button').forEach((button) => button.classList.remove('active'));
    URL.revokeObjectURL(dimensions.src);
  };
  dimensions.onerror = () => { ui.uploadDetail.textContent = `已选择 ${(file.size / 1024 / 1024).toFixed(1)} MB 图片`; URL.revokeObjectURL(dimensions.src); };
...
ui.form.addEventListener('submit', async (event) => { event.preventDefault(); ui.message.textContent = ''; const job = readJobInput(); if (!job.key) { ui.message.textContent = '主创作台尚未保存 RunningHub API Key。'; return; } if (!job.file) { ui.message.textContent = '请先上传待消除图片。'; return; } if (!job.prompt) { ui.message.textContent = '请先写下消除说明。'; ui.prompt.focus(); return; } if (!window.JSZip) { ui.message.textContent = 'ZIP 解压组件加载失败，请检查网络后刷新。'; return; } if (runningTaskKeys.size >= MAX_CONCURRENT_TASKS) { ui.message.textContent = `最多同时运行 ${MAX_CONCURRENT_TASKS} 个任务，请等待任一任务完成后再提交。`; return; }
  ...
  try { updateTaskRecord(record, '正在上传图片…', '正在上传待处理图片至 RunningHub'); const fileName = await uploadReference(job.file, job.key); updateTaskRecord(record, '正在提交任务…', 'RunningHub 已接收处理素材'); const task = await submitTask(job, fileName); if (!task.taskId) throw new Error(task.errorMessage || 'RunningHub 未返回 taskId。'); record.taskId = task.taskId; updateTaskRecord(record, task.status === 'QUEUED' ? '正在排队…' : '正在云端处理…', 'MiniMax H3 正在消除画面元素'); const finalTask = String(task.status).toUpperCase() === 'SUCCESS' ? task : await pollTask(task.taskId, record); if (!record.cancelRequested) await renderResults(finalTask, record); }
